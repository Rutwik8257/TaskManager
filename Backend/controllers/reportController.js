...
const exportUsersReport = async (req, res) => {
  try {
    ...
    console.log(`✅ Fetched ${userTasks.length} tasks`);
    ...
    userTasks.forEach(task => {
      ...
      assignedTo.forEach(assignedUser => {
        if (userTaskMap[assignedUser._id]) {
          ...
          if (task.status === 'In Progress') userTaskMap[assignedUser._id].inProgressTasks += 1;
          if (task.status === 'Completed') userTaskMap[assignedUser._id].completedTasks += 1;
        }
      });
    });
